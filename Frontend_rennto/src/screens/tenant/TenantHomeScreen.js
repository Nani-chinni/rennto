...
if (Platform.OS === "android" && UIManager.setLayoutAnimationEnabledExperimental && !global._IS_FABRIC_) {
...
export default function TenantHomeScreen({ route }) {
  ...
  const unreadNotificationCount = bookingCtx?.unreadNotificationCount || 0;
  const fetchUnreadCount = bookingCtx?.fetchUnreadCount;
  const tenantUnreadCount = Math.max(0, unreadNotificationCount);
  const tenantBadgeText = tenantUnreadCount > 99 ? "99+" : `${tenantUnreadCount}`;
  ...
  const [propertyStructure, setPropertyStructure] = useState(null);
  ...
  // Load the owner's building layout as soon as the joined property is known,
  // instead of only when the Change Floor card is tapped. That tap fired an
  // un-awaited fetch and opened the modal in the same breath, so the floor
  // dropdown rendered against an empty layout.
  useEffect(() => {
    if (isJoined && joinedProperty?.owner_id && !propertyStructure) {
      fetchPropertyStructure(joinedProperty);
    }
  }, [isJoined, joinedProperty?.owner_id]);

  // Clear the cached layout only once the tenant really has no property. The
  // vacate effect above re-runs on every refresh, and clearing it from there
  // could wipe the layout out from under an open Change Floor modal.
  useEffect(() => {
    if (!isJoined) {
      setPropertyStructure(null);
    }
  ...
  const handleAccommodationSubmit = async (payload) => {
    try {
      ...
      // /api/tenantdetails/ returns the owner under `owner`. That fallback was
      // missing here (the vacate flow has it), so on those payloads both
      // ownerPhone and owner_id went out empty and the backend had no owner to
      // notify -- the request silently went nowhere.
      const ownerPhoneNum =
        joinedProperty?.owner_phone ||
        joinedProperty?.ownerPhone ||
        joinedProperty?.owner ||
        joinedProperty?.contact ||
        "";

      const ownerId = joinedProperty?.owner_id || ownerPhoneNum;

      // Never report success when there is no owner to send to.
      if (!ownerPhoneNum && !ownerId) {
        console.warn(
          "[ACCOMMODATION] No owner identifier on joinedProperty. Keys:",
          Object.keys(joinedProperty || {})
        );
        throw new Error(
          "Could not identify your property owner. Please reopen the app and try again."
        );
      }

      const changeTypeLabel = payload.changeType === "FLOOR"
        ? "Floor"
        : payload.changeType === "ROOM"
          ? "Room"
          : "Bed";

      // The modal offers labels like "2nd Floor", "Room 204", "Bed 2"; the API
      // stores bare numbers.
      const numeric = (v) => (v ? String(v).replace(/[^0-9]/g, "") : "");

      // Go through the existing-tenant request endpoint rather than
      // /api/send-owner-notification/. That endpoint only writes a Notification
      // row: the backend sends the owner no push for it at all, and the owner's
      // notification screen reads owner_requests / vacate / hostel-change --
      // never /api/notifications/ -- so the row was invisible and there was
      // nothing for the owner to approve. This one creates a real pending
      // request, pushes to the owner's device, and broadcasts on both sockets
      // the owner app already listens on.
      const res = await fetchWithAuth(`${BASE_URL}/api/existing_tenant_request/`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tenant_phone: tenantPhoneNum,
          owner_id: ownerId,
          owner_phone: ownerPhoneNum,
          property_name: joinedProperty?.property_name || joinedProperty?.name || "",
          property_type: joinedProperty?.property_type || joinedProperty?.type || "",
          check_in: new Date().toISOString().split("T")[0],
          check_out: "N/A",
          sharing: "",
          flat: payload.requestedFlat || "",
          section: payload.requestedSection || "",
          is_existing_tenant: true,
          requested_floor: numeric(payload.requestedFloor),
          requested_room: numeric(payload.requestedRoom),
          requested_bed: numeric(payload.requestedBed),
        }),
      });
      ...
      if (!res.ok) {
        throw new Error(
          data.error || data.message || "Failed to send the request to your owner."
        );
      }
      ...
      if (data.existing) {
        Alert.alert(
          "Request Pending ⏳",
          data.message ||
            "You already have a pending request for this property. Please wait for the owner to respond."
        );
        return;
      }
      ...
      Alert.alert(
        "Request Submitted ✅",
        `Your ${changeTypeLabel.toLowerCase()} change request has been sent to the property owner for approval.`
      );
    } catch (e) {
      console.warn("Accommodation change error:", e);
  ...
  const registerForPushNotifications = async () => {
    ...
    try {
      ...
      if (!pushToken) {
        console.warn(
          "[push] No Expo push token (Expo Go, denied permission, or missing projectId) - not saving to backend"
        );
        return;
      }
    ...
    <SafeAreaProvider>
      ...
      >
          ...
          {isJoined && joinedProperty && (joinedProperty.property_name || joinedProperty.name) !== "N/A" ? (
            <>
              ...
              <View style={{ paddingHorizontal: 16, marginTop: 14, marginBottom: 6 }}>
                ...
                >
                  <LinearGradient
                    colors={showProperties ? ['#F8FAFC', '#F1F5F9'] : ['#8B5CF6', '#6D28D9']}
                    start={{ x: 0, y: 0 }}
                    end={{ x: 1, y: 0 }}
                    style={{
                      flexDirection: "row",
                  ...
                  >
                    <View style={{ flexDirection: "row", alignItems: "center", flex: 1 }}>
                      <View style={{
                        ...
                        height: 48,
                        borderRadius: 24,
                        backgroundColor: showProperties ? "rgba(124, 58, 237, 0.1)" : "rgba(255, 255, 255, 0.2)",
                        justifyContent: "center",
                        alignItems: "center",
                        marginRight: 14,
                      }}>
                        <Ionicons
                          name={showProperties ? "home" : "business"}
                          size={24}
                          color={showProperties ? "#7C3AED" : "#FFFFFF"}
                        />
                      </View>
                      <View style={{ flex: 1, paddingRight: 8 }}>
                        <Text style={{
                          fontSize: 18,
                          fontWeight: "700",
                          color: showProperties ? "#1E293B" : "#FFFFFF",
                          marginBottom: 4,
                        }}>
                        ...
                        <Text style={{
                          fontSize: 13,
                          color: showProperties ? "#64748B" : "rgba(255, 255, 255, 0.8)",
                          fontWeight: "500",
                        }}>
                          {showProperties ? "Check your current booking details" : "Browse available spaces"}
                        </Text>
                      </View>
                    </View>
                    ...
                    <View style={{
                      flexDirection: "row",
                      alignItems: "center",
                      backgroundColor: showProperties ? "#7C3AED" : "#FFFFFF",
                      ...
                      <Text style={{
                        color: showProperties ? "#FFFFFF" : "#7C3AED",
                        fontWeight: "700",
                        fontSize: 13,
                        marginRight: 4,
                      }}>
                        {showProperties ? "CLOSE" : "VIEW"}
                      </Text>
                      <Ionicons
                        name={showProperties ? "chevron-up" : "chevron-forward"}
                        size={16}
                        color={showProperties ? "#FFFFFF" : "#7C3AED"}
                      />
                    </View>
              ...
              {/* AVAILABLE PROPERTY LISTINGS (When active search or toggle enabled) */}
              ...
              {!showProperties && (
                <View style={homeStyles.myStayContainer}>
                  ...
                  <View style={homeStyles.myStayGrid}>
                    ...
                    {/* 3. Vacate Property Card: Light Red -> Light Pink */}
                    <TouchableOpacity
                    ...
                    {/* 4. Change Floor Card (Hostel & Apartment): Light Purple -> Lavender */}
        ...
        <AccommodationChangeModal
          ...
          currentAllocation={{
            // /api/tenantdetails/ returns floor_number / room_number. The keys
            // read here before (floor / floor_no / room / room_no) do not exist
            // on that payload, so the modal never knew where the tenant already
            // was: it could not preselect their floor, and their own bed showed
            // as taken.
            floor:
              joinedProperty?.floor_number ??
              joinedProperty?.floor ??
              joinedProperty?.floor_no ??
              "",
            room:
              joinedProperty?.room_number ??
              joinedProperty?.room ??
              joinedProperty?.room_no ??
              "",
            bed: joinedProperty?.bed ?? joinedProperty?.bed_no ?? "",
          }}
          buildingLayout={propertyStructure?.building_layout || []}
          propertyType={joinedProperty?.property_type || joinedProperty?.type || "hostel"}
          loadingLayout={loadingStructure}
        />
...
export function PropertyDetailsScreen(props) {
  ...
  const bookingContext = useContext(BookingContext);
  const { requests = [], setRequests, isJoined, joinedProperty, tenantProfile } = bookingContext || {};
      ...
      if (isJoined && (!joinedProperty || joinedProperty.property_name?.trim() !== property.name?.trim())) {
        ...
        if (storedPhone && property.id) {
          try {
            ...
            // "approved_request": continue straight to booking; the approval was
            // already shown when the booking form was opened.
          } catch (e) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: "#fff" }]}>
      ...
      >
        {tenantProfile === null || tenantProfile === undefined ? (
          <View style={[styles.mainImage, { backgroundColor: '#f1f5f9', justifyContent: 'center', alignItems: 'center' }]}>
          ...
          <View style={styles.row}>
            ...
            <View style={[
              ...
              ((requestStatus === "accepted" ||
                requestStatus === "allotted") && isAadhaarUploaded) && {
                backgroundColor: "#3498db"
              },
              (requestStatus === "pending" || ((requestStatus === "accepted" || requestStatus === "allotted") && !isAadhaarUploaded)) && { backgroundColor: "#f39c12" },
              ...
              ]}>
                {
                  requestStatus === "completed" ||
                    ...
                    : (requestStatus === "accepted" ||
                      ...
                      : requestStatus === "pending" || ((requestStatus === "accepted" || requestStatus === "allotted") && !isAadhaarUploaded)
                        ? (t("pending") || "Pending")
                    ...
                    <TouchableOpacity
                      ...
                      onPress={async () => {
                        try {
                          ...
                          // 4. Room Approval Notification
                          if (r.type === "room_approval") {
                            // Show to tenant as unseen notification
                            return isUnseen;
                          }
...
const homeStyles = StyleSheet.create({
  ...
  joinedLocationWrapper: {
    flexDirection: "row",
    alignItems: "center",
    // Same chip treatment as joinedStatusBadge above: the address sat directly
    // on the property photo, so anything busy behind it made it unreadable.
    // alignSelf keeps the chip hugging the text instead of spanning the hero.
    alignSelf: "flex-start",
    maxWidth: "100%",
    backgroundColor: "rgba(108, 99, 255, 0.8)",
    paddingHorizontal: 10,
