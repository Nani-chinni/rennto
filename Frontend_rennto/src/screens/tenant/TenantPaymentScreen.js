...
const TenantPaymentScreen = () => {
  ...
  const fetchPaymentDetails = async (isBackground = false, silentError = false) => {
    try {
  ...
  const handleUPIPayment = async (app) => {
    ...
    // Use the owner's UPI ID if available, otherwise use a fallback for testing
    const targetUpiId = paymentData?.upiId || 'test@ybl';
    ...
    const upiUrl = `upi://pay?pa=${targetUpiId}&pn=${encodeURIComponent(name)}&am=${amount}&cu=INR&tn=${encodeURIComponent(note)}`;
    const schemes = {
      'PhonePe': `phonepe://pay?pa=${targetUpiId}&pn=${encodeURIComponent(name)}&am=${amount}&cu=INR&tn=${encodeURIComponent(note)}`,
      'Google Pay': `tez://upi/pay?pa=${targetUpiId}&pn=${encodeURIComponent(name)}&am=${amount}&cu=INR&tn=${encodeURIComponent(note)}`,
      'Paytm': `paytmmp://pay?pa=${targetUpiId}&pn=${encodeURIComponent(name)}&am=${amount}&cu=INR&tn=${encodeURIComponent(note)}`,
      'BHIM': `bhim://pay?pa=${targetUpiId}&pn=${encodeURIComponent(name)}&am=${amount}&cu=INR&tn=${encodeURIComponent(note)}`,
    };
  ...
  const handleCashPayment = async () => {
    ...
    try {
      ...
      setCashDescription('');
      fetchPaymentDetails(false, true);
    } catch (error) {
  ...
  const handleSendToOwner = async () => {
    ...
    try {
      ...
      if (paymentAmount) formData.append('amount', String(paymentAmount));
      if (paymentData?.txn_ref) formData.append('txn_ref', String(paymentData.txn_ref));
      if (screenshotDescription.trim()) formData.append('description', String(screenshotDescription));

      // 1. Upload the screenshot
      const uploadRes = await fetchWithAuth(`${BASE_URL}/api/upload-payment-screenshot/`, {
        method: 'POST',
        body: formData,
      });
      ...
      const notifRes = await fetchWithAuth(`${BASE_URL}/api/send-owner-notification/`, {
        method: 'POST',
        body: {
          owner_phone: paymentData.ownerId,
      ...
      if (!notifRes.ok) {
         console.log("Notification send failed:", await notifRes.text());
      }
      ...
      const serverError = error.message || "Unknown error";
      Alert.alert(t('error') || 'Error', `Upload Failed: ${serverError}`);
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleAcknowledgeSuccess = async () => {
    ...
    try {
      ...
      fetchPaymentDetails(false, true); // Refresh to update Paid status
    } catch (error) {
      setShowSuccessModal(false);
    }
  };
