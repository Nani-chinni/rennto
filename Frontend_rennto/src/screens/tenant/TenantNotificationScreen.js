...
const TenantNotificationScreen = () => {
  ...
  const submitIdentityProof = async () => {
    ...
    if (!activePhone) {
      Alert.alert(
        "Error",
        "Tenant details not found. Please log in again."
      );
      return;
    }
    ...
    if (!selectedFile || !selectedBackFile || !selectedItem || !aadharId) {
      Alert.alert(
        "Error",
        "Please enter Aadhaar ID and upload Aadhaar Front & Back images."
      );
      return;
    }

    const cleanAadharId = String(aadharId).trim();

    if (!/^\d{12}$/.test(cleanAadharId)) {
      Alert.alert(
        "Error",
    ...
    if (!selectedFile.uri || !selectedBackFile.uri) {
      Alert.alert(
        "Error",
        "Please select Aadhaar Front and Back images again."
      );
      return;
    }
    ...
    try {
      ...
      formData.append("phone", String(activePhone));
      formData.append("aadhar_id", cleanAadharId);
      ...
      console.log("Submitting identity proof...");

      let res;
      try {
        const fullUrl = `${BASE_URL}/api/tenant/submit_verification/`;
        res = await fetchWithAuth(fullUrl, {
          method: 'POST',
          body: formData,
        });
      } catch (error) {
      ...
      if (res.ok) {
        ...
        try {
          const currentPhone =
            phone || tenantPhone || activePhone;

          const storageKey =
            `joinedRequestIds_${currentPhone}`;
      ...
      console.log(
        "Error submitting identity proof:",
        err
      );
      ...
      Alert.alert(
        "Error",
        err?.message ||
        "Could not submit identity proof. Please try again."
      );
