...
import { clearCache } from "@/src/utils/CacheHelper";
...
export default function OwnerProfile({ navigation }) {
  ...
  const [aiPan] = useState(() => new Animated.ValueXY());
  ...
  const [profitPulseAnim] = useState(() => new Animated.Value(1));
  const [profitGlowOpacity] = useState(() => new Animated.Value(0));
  ...
  const handleLogout = async () => {
    Alert.alert(t("logout") || "Logout", t("logout_confirm") || "Are you sure you want to logout?", [
     ...
     { text: t("logout") || "Logout", onPress: async () => { await AsyncStorage.multiRemove(["ownerPhone", "userToken", "userRole", "loggedInOwnerAccounts", "selectedAccountId", "owner_accounts_cache", "notificationSeenIds", "notificationClearedIds"]); await clearCache(); navigation.reset({ index: 0, routes: [{ name: 'RoleSection', params: { skipSplash: true } }] }) } }
    ]);
  };
        ...
        <View style={styles.section}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={async () => {
                if (!editableOwner || !editableOwner.phone) {
                  navigation.navigate('OwnerRegistrationScreen', { isAddingAccount: false });
                } else {
                  await loadAccounts();
