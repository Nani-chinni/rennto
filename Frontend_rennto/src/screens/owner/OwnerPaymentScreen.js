...
const OwnerPaymentScreen = () => {
  ...
  // silentError: refresh right after an action that already showed its own result popup.
  const fetchPayments = async (isBackground = false, silentError = false) => {
    try {
      ...
      const formattedData = data.map((item, index) => {
        ...
        return {
          ...
          // Real date parts, kept for filtering. The localised strings below
          // are for display only - Android's ICU renders September as "Sept"
          // for en-IN while iOS renders "Sep", so matching on them is unsafe.
          dateDay: isNaN(date.getTime()) ? null : date.getDate(),
          dateMonth: isNaN(date.getTime()) ? null : date.getMonth(),
          dateYear: isNaN(date.getTime()) ? null : date.getFullYear(),
      ...
      // Background polling runs every 10s; only alert for user-triggered loads.
      if (!isBackground && !silentError) {
  ...
  const handleSaveBank = async () => {
    ...
    if (upiErr || phoneErr || !hasQr) {
      ...
      // Field errors are shown inline above; no extra popup needed.
      return;
    }
  ...
  const handleManualStatusChange = async (tenantId, newStatus, txn_ref) => {
    ...
    try {
      ...
      fetchPayments(false, true); // Refresh data
  ...
  const submitRejectPayment = async () => {
    ...
    try {
      ...
      await fetchPayments(false, true); // Refresh full data
  ...
  const handleSetReminder = async (proof) => {
    ...
    fetchPayments(false, true); // Refresh in background
  };
  ...
  const filteredTenants = tenantData.filter(tenant => {
    // Match on real date parts, not on localised date text. The old test built
    // the needle "Sep 2026" from selectedMonth and looked for it inside
    // dueDate. On Android, toLocaleDateString('en-IN') renders September as
    // "11 Sept 2026" (four letters, per en-GB/en-IN CLDR) while iOS renders
    // "11 Sep 2026" - so the needle never matched on Android and every payment
    // was filtered out, leaving the list empty in September.
    const hasDateParts =
      typeof tenant.dateYear === 'number' && typeof tenant.dateMonth === 'number';

    if (filterByDate) {
      if (hasDateParts) {
        if (
          tenant.dateDay !== selectedDate.getDate() ||
          tenant.dateMonth !== selectedDate.getMonth() ||
          tenant.dateYear !== selectedDate.getFullYear()
        ) return false;
      } else {
      ...
      const monthIndex = MONTH_NAMES.findIndex(m => selectedMonth.startsWith(m));
      const selectedYear = parseInt(selectedMonth.split(' ')[1], 10);

      if (hasDateParts && monthIndex >= 0 && !isNaN(selectedYear)) {
        if (tenant.dateMonth !== monthIndex || tenant.dateYear !== selectedYear) return false;
      } else {
