...
export default function BuildingScreen({ route }) {
  ...
  useFocusEffect(
    useCallback(() => {
      fetchUnreadCount?.(phone, "owner");
    }, [fetchUnreadCount, phone])
  );

  const ownerUnreadCount = Math.max(0, unreadNotificationCount || 0);
...
const [isAddingFloor, setIsAddingFloor] = useState(false);
  ...
  const [floorPulse] = useState(() => new Animated.Value(0.8));
...
useEffect(() => {
  ...
  // Occupancy data (building layout + tenant rows) can be requested from several
  // places at once (mount, refreshTrigger bumps, screen focus). Only the newest
  // request may update state, so a slower, older response can never overwrite
  // fresher tenant data.
  const occupancyRequestSeq = useRef(0);
  const lastOccupancyFetchAt = useRef(0);
  const loadOccupancyDataRef = useRef(null);

  useEffect(() => {
    if (!phone) return;

    const fetchData = async () => {
      const requestId = ++occupancyRequestSeq.current;
      lastOccupancyFetchAt.current = Date.now();
      const isStale = () => requestId !== occupancyRequestSeq.current;
      try {
    ...
    loadOccupancyDataRef.current = fetchData;
    fetchData();
   }, [phone, refreshTrigger, selectedAccount, isConnected]);

  // Home is a tab screen that stays mounted, so the effect above does not run
  // again when the owner returns to it (e.g. after approving/allotting a tenant
  // on another screen, or after the tenant joins from their app). Re-fetch the
  // same data whenever the screen is focused again after being left.
  useEffect(() => {
    ...
    const unsubscribeFocus = navigation.addListener("focus", () => {
      ...
      wasBlurred = false;
      // Skip if a fetch was started moments ago (e.g. by refreshTrigger right
      // before navigating here), so no duplicate request is sent.
      if (Date.now() - lastOccupancyFetchAt.current < 1500) return;
      loadOccupancyDataRef.current?.();
    });
    return () => {
      unsubscribeBlur();
      unsubscribeFocus();
                        ...
                        <View style={styles.roomCardGrid}>
                          {(item.units || [])
                            ...
                            .map((unit, i) => {
                              ...
                              // Get first tenant info (latest data from the tenant list)
                              const primaryTenant = roomTenants[0];
                              if (stayType !== "hostel" && primaryTenant?.name) {
                                bedsLabel = `${bedsLabel} • ${primaryTenant.name}`;
                              }
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <Text style={styles.bedsLabelText} numberOfLines={1}>{bedsLabel}</Text>
