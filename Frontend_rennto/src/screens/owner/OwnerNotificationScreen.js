...
const OwnerNotificationScreen = ({ route }) => {
  ...
  const fetchRequests = async () => {
    ...
    try {
      ...
      const mappedData = (Array.isArray(data) ? data : []).map(item => {
        if (item.is_existing_tenant || item.type === "existing_tenant" || item.type === "accommodation_change") {
          return {
            ...item,
        ...
        if (item.is_vacate_request || item.type === "vacate_request" || item.type === "VACATE_REQUEST") {
          return {
            ...item,
            type: "vacate_request",
            title: "Vacate Property Request",
            tenant_name: item.name || item.tenant_name || "Tenant",
            message: item.message || `${item.name || item.tenant_name || "Tenant"} has requested to vacate the property.`,
      ...
      try {
        ...
        if (vRes.ok) {
          ...
          vacateData = (Array.isArray(vJson) ? vJson : []).map(v => ({
            ...
            type: "vacate_request",
            title: "Vacate Property Request",
            tenant_name: v.tenant?.name || v.name || "Tenant",
            message: v.message || `${v.tenant?.name || v.name || "Tenant"} has requested to vacate the property.`,
            remarks: v.remarks || "",
          }));
        }
      } catch (vErr) {
      ...
      rawCombined.forEach(item => {
        const rawId = String(item.db_id || item.id || item.request_id || "").replace(/^(vac_|hc_|req_|exreq_)/, "");
        const itemType = (item.type || (item.is_vacate_request ? "vacate_request" : item.is_existing_tenant ? "accommodation_change" : "join_request")).toLowerCase();
        const key = `${itemType}_${rawId}`;
        if (!seenMap.has(key)) {
  ...
  // showResultFor: pass the request title to show one result popup after the
  // server responds (used by the card buttons that previously showed it too early).
  const handleAction = async (action, id, isExisting = false, showResultFor = null) => {
    try {
      ...
      if (showResultFor) {
        if (action === "accepted") {
          Alert.alert("Approved ✅", `${showResultFor} approved successfully.`);
        } else {
          Alert.alert("Declined ✖", `${showResultFor} declined.`);
        }
      }
    } catch (error) {
  ...
  const renderCard = (item) => {
    ...
    if (item.type === "accommodation_change") {
      const iconName = "swap-horizontal-outline";
      const iconColor = "#7C3AED";
      const iconBg = "#F3E8FF";
      const tenantName = item.tenant_name || item.name || "Tenant";
      const propertyName = item.propertyName || item.property_name || "Property";
      const reqId = item.db_id || item.id;
      const isPending = (item.status || "pending").toLowerCase() === "pending";

      const requestedSummary = item.requested_location || [
        item.floor || item.requested_floor ? `Floor ${item.floor || item.requested_floor}` : null,
        item.room || item.requested_room || item.flat || item.requested_flat ? `Room ${item.room || item.requested_room || item.flat || item.requested_flat}` : null,
        item.bed || item.requested_bed ? `Bed ${item.bed || item.requested_bed}` : null,
      ].filter(Boolean).join(", ");

      return (
        <View key={`acc-change-${item.id}`} style={styles.card}>
          <View style={styles.cardHeader}>
            <View style={styles.userInfo}>
              <View style={[styles.avatar, { backgroundColor: iconBg }]}>
                <Ionicons name={iconName} size={20} color={iconColor} />
              ...
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                  ...
                  <View style={{ backgroundColor: "#EDE9FE", paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4 }}>
                    <Text style={{ fontSize: 10, fontWeight: "700", color: "#6D28D9" }}>Existing Tenant</Text>
                  </View>
                </View>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 6, marginTop: 2 }}>
                  <Text style={styles.userPhone} numberOfLines={1}>{tenantName} {item.phone ? `(${item.phone})` : ""}</Text>
                  <Text style={styles.timeDot}>•</Text>
                  <Text style={styles.timeText}>{formatDate(item.created_at || item.createdAt)}</Text>
                </View>
              </View>
          ...
          <View style={{ marginTop: 10, paddingHorizontal: 4 }}>
            ...
            <View style={{ backgroundColor: "#F8FAFC", padding: 10, borderRadius: 10, marginTop: 4, borderWidth: 1, borderColor: "#E2E8F0" }}>
              <Text style={{ fontSize: 13, color: "#1E293B", fontWeight: "600", lineHeight: 18 }}>
                {item.message || `${tenantName} requested an accommodation change.`}
              </Text>
            </View>

            {/* Location Transition Tags */}
            <View style={{ flexDirection: "row", alignItems: "center", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
              ...
              {requestedSummary ? (
                <View style={{ backgroundColor: "#EDE9FE", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6, borderWidth: 1, borderColor: "#DDD6FE" }}>
          ...
          {isPending && (
            <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
              <TouchableOpacity
                style={{ flex: 1, backgroundColor: "#F3F4F6", paddingVertical: 10, borderRadius: 8, alignItems: "center", borderWidth: 1, borderColor: "#E5E7EB" }}
                onPress={() => {
                  handleAction("rejected", reqId, true, item.title || "Request");
                }}
              >
                <Text style={{ color: "#4B5563", fontWeight: "700", fontSize: 13 }}>Decline</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  handleAction("accepted", reqId, true, item.title || "Request");
                }}
              >
                <Text style={{ color: "#fff", fontWeight: "700", fontSize: 13 }}>Accept</Text>
              </TouchableOpacity>
