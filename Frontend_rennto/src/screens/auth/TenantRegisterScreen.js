 ...
 await AsyncStorage.setItem("userRole", "tenant");
        ...
        try {
          ...
          if (finalStatus !== "granted") {
            console.log("NOTIFICATION PERMISSION DENIED - continuing login without push token");
          } else {
...
// A push token is optional metadata on the registration form. This used to
// `return`, which returns from handleRegister itself -- not just this block --
// so denying (or not yet granting) the notification permission silently
// abandoned the whole registration before the API was ever called.
if (finalStatus !== "granted") {

  console.log(
    "NOTIFICATION PERMISSION DENIED - registering without a push token"
  );
...
console.log(
  "========== REGISTER PUSH END =========="
      ...
      if (response.status === 201 || response.status === 200) {
        ...
        // SAVE ROLE - same key the existing-tenant login path writes above.
        // Without it a newly registered tenant has no userRole, so the
        // notification socket joins the owner channel instead of the tenant one.
        await AsyncStorage.setItem("userRole", "tenant");

        // SAVE TENANT ID
        // Read the id defensively, the same way the existing-tenant path above
        // does. `data.data.id` alone threw a TypeError whenever the server
        // replied with any other shape, and the throw landed in the outer catch
        // -- so a registration that actually succeeded looked like a failure and
        // never navigated.
        const newTenantId =
          data?.data?.id ??
          data?.tenant?.id ??
        ...
        if (newTenantId !== undefined && newTenantId !== null) {
          await AsyncStorage.setItem("tenantId", String(newTenantId));
        }

        console.log("SAVED TENANT ID:", newTenantId);
        ...
        // App.js replaces console.log with a no-op, so every failure here was
        // completely silent - the button appeared to do nothing at all.
        Alert.alert(
          t("error") || "Error",
          data.errors
