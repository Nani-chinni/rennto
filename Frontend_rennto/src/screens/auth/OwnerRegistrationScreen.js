...
const getMaxDigitsForPropertyType = (propertyType) => {
  if (!propertyType) return 5;
...
const formatIndianNumber = (rawDigits) => {
  if (rawDigits === null || rawDigits === undefined || rawDigits === "") return "";
  const cleaned = String(rawDigits).replace(/\D/g, "");
  if (!cleaned) return "";
  if (cleaned.length <= 3) {
    return cleaned;
  }
  ...
  const rev = remaining.split("").reverse().join("");
  const groups = rev.match(/.{1,2}/g) || [];
  const formattedRem = groups.join(",").split("").reverse().join("");
...
export default function OwnerCommercialSection({
  ...
  const handleRentChange = (v, propertyType) => {
    const pType = propertyType || form.stayType;
    const maxDigits = getMaxDigitsForPropertyType(pType);
    const cleaned = (v || "").replace(/[^0-9]/g, "").slice(0, maxDigits);
    setForm((prev) => ({ ...prev, rent: cleaned }));
    if (cleaned && Number(cleaned) > 0) {
      setErrors((prev) => {
        if (!prev.rent) return prev;
        const copy = { ...prev };
        delete copy.rent;
        return copy;
      });
  ...
  const handleStayTypeChange = (nextType) => {
    ...
    const maxDigits = getMaxDigitsForPropertyType(nextType);
    const currentRentDigits = (form.rent || "").replace(/\D/g, "");
    const revalidatedRent = currentRentDigits.slice(0, maxDigits);
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      ...
      if (isMultiple) {
        ...
        result.assets.forEach((asset) => {
          if (asset.fileSize && asset.fileSize > 5242880) {
            hasSizeError = true;
            newErrors[`document_${imagePickerTarget}`] =
              t("image_size_limit");
          } else if (asset) {
            if (!asset.mimeType) {
  ...
  const scrollToFirstError = (newErrors) => {
    ...
    if (errorKeys.length > 0) {
      if (Platform.OS === 'android') {
        ToastAndroid.show(t("please_fill_details") || "Please fill all required details before proceeding.", ToastAndroid.SHORT);
      } else {
        Alert.alert(t("missing_fields") || "Missing Fields", t("please_fill_details") || "Please fill all required details before proceeding.");
      }
      let minY = Number.MAX_VALUE;
      for (const key of errorKeys) {
        if (fieldPositions.current[key] !== undefined) {
          minY = Math.min(minY, fieldPositions.current[key]);
        }
      }
      if (minY !== Number.MAX_VALUE) {
        scrollViewRef.current?.scrollTo({ y: Math.max(0, minY - 50), animated: true });
      } else {
        ...
        onPress: async () => {
          try {
            ...
            if (hasFiles) {
              ...
              formData.append("rent_amount", String(form.rent || ""));
              formData.append("furnishing_type", String(form.furnishingType || ""));

              if (form.documents?.coverImage) {
                const cover = form.documents.coverImage;
                if (cover.uri) {
                  formData.append("cover_image", {
                    uri: Platform.OS === 'ios' ? cover.uri.replace('file://', '') : cover.uri,
                    name: cover.fileName || cover.name || "cover.jpg",
                    type: cover.mimeType || "image/jpeg"
                  });
                }
              }

              if (Array.isArray(form.documents?.homePics)) {
                for (let index = 0; index < form.documents.homePics.length; index++) {
                  const img = form.documents.homePics[index];
                  if (img?.uri) {
                    formData.append("gallery_images", {
                      uri: Platform.OS === 'ios' ? img.uri.replace('file://', '') : img.uri,
                      name: img.fileName || img.name || `gallery_${index}.jpg`,
              ...
              bodyData = formData;
              // Do NOT manually set Content-Type for FormData, fetch will automatically add the multipart boundary
            } else {
              const jsonData = {
                ...submitData,
                building_layout: JSON.stringify(form.floorsData || []),
                rent_amount: form.rent || "",
                furnishing_type: form.furnishingType || "",
              };
              
              if (isAddingAccount) {
                jsonData.action = "register";
              }
            ...
            if (hasFiles) {
              ...
              try {
                response = await fetchWithAuth(fullUrl, {
                  method: 'POST',
                  body: bodyData, // This is FormData
                });
              } catch (error) {
            ...
            if (response.ok) {
              ...
              if (data.status === "active" || data.details?.status === "active") {
                const targetPhone = data.owner_id || data.details?.owner_id || phone;
                ...
                Alert.alert(t("success"), t("registration_successful"), [
                  {
                    ...
                    onPress: () =>
                      navigation.replace("WaitingScreen", {
                        phone: data.owner_id || data.details?.owner_id || phone,
                      }),
                  },
                ]);
              }
              ...
              console.log("REGISTRATION ERROR:", errText);
              Alert.alert(t("error"), errText || t("registration_failed"));
            }
          } catch (error) {
            setIsLoading(false);
                  ...
                  {step === 1 && (
                    <>
                      ...
                      <View style={styles.sectionCard}>
                        ...
                        <Text onLayout={onLayoutField('name')} style={[styles.label, { color: "#111827", fontWeight: "600" }]}>
                        ...
                        >
                          ...
                          <TextInput
                            ...
                            maxLength={30}
                            onChangeText={(v) => {
                              let filtered = v.replace(/[^A-Za-z\s]/g, "");
                        ...
                        <Text onLayout={onLayoutField('idProofType')} style={[styles.label, { color: "#111827", fontWeight: "600" }]}>
                      ...
                      <View style={styles.sectionCard}>
                        ...
                        <Text onLayout={onLayoutField('stayType')} style={[styles.label, { color: "#111827", fontWeight: "600" }]}>
                        ...
                        {form.stayType === "hostel" && (
                          <>
                            <Text onLayout={onLayoutField('hostelName')} style={styles.label}>{t("hostel_name")}</Text>
                            <View
                              style={[
                                styles.inputContainer,
                                styles.inputContainerStep2,
                            ...
                            <Text onLayout={onLayoutField('location')} style={[styles.label, { color: "#111827", fontWeight: "600" }]}>Location <Text style={{ color: "#EF4444" }}>*</Text></Text>
                            <View
                              style={[
                                styles.inputContainer,
                                styles.inputContainerStep2,
                            ...
                            <Text onLayout={onLayoutField('hostelType')} style={styles.label}>{t("hostel_type")}</Text>
                            ...
                            {/* Rent Amount */}
                            <Text onLayout={onLayoutField('rent')} style={styles.label}>Basic Rent / Basic Property Amount</Text>
                            <View style={[styles.inputContainer, styles.inputContainerStep2]}>
                              ...
                              <TextInput
                                ...
                                value={formatIndianNumber(form.rent)}
                                onChangeText={(v) => handleRentChange(v, "hostel")}
                              />
                            </View>
                        ...
                        {form.stayType === "apartment" && (
                          <>
                            <Text onLayout={onLayoutField('apartmentName')} style={styles.label}>{t("apartment_name")}</Text>
                            <View
                              style={[
                                styles.inputContainer,
                                styles.inputContainerStep2,
                            ...
                            <Text onLayout={onLayoutField('location')} style={styles.label}>{t("location")}</Text>
                            <View
                              style={[
                                styles.inputContainer,
                                styles.inputContainerStep2,
                            ...
                            <Text onLayout={onLayoutField('tenantType')} style={styles.label}>{t("tenant_type")}</Text>
                            ...
                            <View style={[styles.inputContainer, styles.inputContainerStep2]}>
                              ...
                              <TextInput
                                ...
                                keyboardType="numeric"
                                value={formatIndianNumber(form.rent)}
                                onChangeText={(v) => handleRentChange(v, "apartment")}
                              />
                            </View>
                            {errors.rent ? <Text style={styles.errorText}>{errors.rent}</Text> : null}
                            {/* Furnishing Type - Apartment only */}
                            <Text onLayout={onLayoutField('furnishingType')} style={styles.label}>Furnishing Type</Text>
                        ...
                        {form.stayType === "commercial" && (
                          <>
                            <Text onLayout={onLayoutField('commercialName')} style={styles.label}>{t("property_name")}</Text>
                            <View
                              style={[
                                styles.inputContainer,
                                styles.inputContainerStep2,
                            ...
                            <Text onLayout={onLayoutField('usage')} style={styles.label}>{t("usage")}</Text>
                            ...
                            <View style={[styles.inputContainer, styles.inputContainerStep2]}>
                              ...
                              <TextInput
                                ...
                                keyboardType="numeric"
                                value={formatIndianNumber(form.rent)}
                                onChangeText={(v) => handleRentChange(v, "commercial")}
                              />
                            </View>
                            {errors.rent ? <Text style={styles.errorText}>{errors.rent}</Text> : null}
                          </>
                        ...
                        {form.stayType !== "" && (
                          <View style={{ marginTop: 8, marginBottom: 4 }}>
                            ...
                            <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 10 }}>
                              ...
                              <View>
                                <Text onLayout={onLayoutField('document_coverImage')} style={{ fontSize: 16, fontWeight: "700", color: "#111827" }}>Cover Image</Text>
                          ...
                          <View style={{ marginTop: 8, marginBottom: 8 }}>
                            ...
                            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                              <View style={{ flexDirection: "row", alignItems: "center" }}>
                                ...
                                <View>
                                  <Text onLayout={onLayoutField('document_homePics')} style={{ fontSize: 16, fontWeight: "700", color: "#111827" }}>Property Gallery</Text>
                              ...
                              {Array.isArray(form.documents.homePics) && form.documents.homePics.length > 0 && (
                                <View style={{ backgroundColor: "#7C3AED", borderRadius: 12, paddingHorizontal: 10, paddingVertical: 4 }}>
                                  <Text style={{ color: "#FFF", fontSize: 12, fontWeight: "700" }}>{form.documents.homePics.length}/30</Text>
                                </View>
                              )}
                            </View>
                            ...
                            >
                              ...
                              <Text style={{ fontSize: 12, color: "#9CA3AF" }}>Maximum 30 Photos • JPG, PNG</Text>
                            </TouchableOpacity>
                            ...
                            {Array.isArray(form.documents.homePics) && form.documents.homePics.length > 0 && (
                              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                                ...
                                {/* Add more button */}
                                {form.documents.homePics.length < 30 && (
                                  <TouchableOpacity
                                    style={{
...
function Step3({ form, onUpdateFloors }) {
  ...
  const [roomSlideAnim] = useState(() => new Animated.Value(SCREEN_HEIGHT));
...
function ApartmentLayout({ form, onUpdateFloors }) {
  ...
  const [slideAnim] = useState(() => new Animated.Value(SCREEN_HEIGHT));
  const [flatSlideAnim] = useState(() => new Animated.Value(SCREEN_HEIGHT));
  const [blinkAnim] = useState(() => new Animated.Value(1));
...
function CommercialLayout({ form, onUpdateFloors }) {
  ...
  const [sectionSlideAnim] = useState(() => new Animated.Value(SCREEN_HEIGHT));
  const [blinkAnim] = useState(() => new Animated.Value(1));
