...
export default function WaitingScreen({ navigation, route }) {
  const { t } = useLanguage();
  const { accounts, refreshAccounts, switchAccount } = useContext(OwnerAccountContext);
    ...
    <View style={styles.container}>
      ...
      {status === "suspend" ? (
        ...
        <>
          ...
          {/* Optional Back Button (if they have an active property to go back to) */}
          {accounts && accounts.some(a => a.status === "active" && String(a.id) !== String(phone)) && (
            <TouchableOpacity
              style={styles.switchAccountsButton}
              onPress={async () => {
                const activeAcc = accounts.find(a => a.status === "active" && String(a.id) !== String(phone));
                if (activeAcc) {
                  const res = await switchAccount(activeAcc.id);
                  if (res.success) {
                    navigation.reset({
                      index: 0,
                      routes: [{
                        ...
                        params: {
                          ...
                          destinationParams: { phone: activeAcc.id },
                        }
                      }]
                    });
                  } else {
                    Alert.alert("Error", res.error || "Failed to go back");
                  }
                }
              }}
            >
              <Ionicons name="arrow-back" size={20} color="#7C3AED" style={{ marginRight: 8 }} />
              <Text style={styles.switchAccountsButtonText}>
                {t("back_to_current_account") || "Back to Current Account"}
              </Text>
            </TouchableOpacity>
          )}
