  ...
  const handleAccommodationSubmit = async (payload) => {
    try {
      ...
      const ownerPhoneNum = 
        ...
        joinedProperty?.owner_id || 
        "";

      const messageContent = `${joinedProperty?.tenant_name || tenantPhoneNum || "Tenant"} requested ${payload.changeType.toLowerCase()} change to ${payload.requestedFloor || payload.requestedRoom || payload.requestedBed || "new allocation"}.\nReason: ${payload.reason || "N/A"}`;
      ...
      const requestPayload = {
        ...
        requestedFloor: payload.requestedFloor,
        requestedRoom: payload.requestedRoom,
        requestedBed: payload.requestedBed,
        reason: payload.reason,
        owner_id: joinedProperty?.owner_id || ownerPhoneNum,
        title: `${payload.changeType} Change Request`,
        message: messageContent,
      };

      // Call the backend API instead of the mock function
      const res = await fetchWithAuth(`${BASE_URL}/api/accommodation-change/create/`, {
        method: "POST",
        body: requestPayload
      });
      ...
      // Also call mock if needed for local UI update
      if (submitOwnerRequest) {
        submitOwnerRequest({ ...requestPayload, status: "pending" });
      }

      Alert.alert(
        "Request Submitted",
        `Your ${payload.changeType.toLowerCase()} change request has been sent to the property owner.`
      );
    } catch (e) {
      console.log("Accommodation change error:", e);
