...
def check_owner_status(request, phone):
    try:
        ...
        if request.jwt_payload.get('role') == 'owner':
            owner_obj = getattr(request, 'owner_account', None) or request.custom_user
            target_owner = CommonService.get_owner(phone)
            if not owner_obj or not target_owner or not CommonService.is_same_or_authorized_owner(owner_obj, target_owner):
                return Response({"error": "You do not have permission to perform this action."}, status=status.HTTP_403_FORBIDDEN)
            phone = target_owner.owner_id
...
# =====================================================================
# ACCOMMODATION CHANGE REQUEST ENDPOINTS
# =====================================================================

@api_view(['POST'])
@jwt_required()
def create_accommodation_change_request(request):
    try:
        from HAC.models import Tenent, Owners, StayHostelDetails, AccommodationChangeRequest
        from HAC.services.notification_service import NotificationService
        ...
        owner = Owners.objects.filter(owner_id=data.get('owner_id')).first()
        if not owner:
            return Response({"error": "Owner not found"}, status=status.HTTP_404_NOT_FOUND)
            
        # Get the tenant's current property based on owner ID (simplified)
        property = StayHostelDetails.objects.filter(owner_id=owner.owner_id).first()
        if not property:
            return Response({"error": "Property not found"}, status=status.HTTP_404_NOT_FOUND)

        change_request = AccommodationChangeRequest.objects.create(
            tenant=tenant,
            property=property,
            owner=owner,
            change_type=data.get('changeType', 'UNKNOWN'),
            current_floor=data.get('currentFloor'),
            current_room=data.get('currentRoom'),
            current_bed=data.get('currentBed'),
            requested_floor=data.get('requestedFloor'),
            requested_room=data.get('requestedRoom'),
            requested_bed=data.get('requestedBed'),
            reason=data.get('reason', '')
        )
        
        NotificationService.send_owner_notification(
            owner_phone=owner.phone,
            title=data.get('title', 'Accommodation Change Request'),
            message=data.get('message', 'A tenant has requested an accommodation change.'),
            type="ACCOMMODATION_CHANGE",
            related_id=change_request.id
        )
        
        from HAC.serializers import AccommodationChangeRequestSerializer
        serializer = AccommodationChangeRequestSerializer(change_request)
        return Response({"status": True, "request": serializer.data}, status=status.HTTP_201_CREATED)
    except Exception as e:
        return Response({"status": False, "error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

@api_view(['GET'])
@jwt_required()
def get_tenant_accommodation_requests(request, tenant_phone):
    try:
        from HAC.models import AccommodationChangeRequest
        from HAC.serializers import AccommodationChangeRequestSerializer
        
        if request.jwt_payload.get('role') != 'tenant' or request.jwt_payload.get('phone') != tenant_phone:
            return Response({"error": "Unauthorized access"}, status=status.HTTP_403_FORBIDDEN)
            
        requests_list = AccommodationChangeRequest.objects.filter(tenant__phone=tenant_phone)
        serializer = AccommodationChangeRequestSerializer(requests_list, many=True)
        return Response({"requests": serializer.data}, status=status.HTTP_200_OK)
    except Exception as e:
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
@api_view(['GET'])
@jwt_required()
def get_owner_accommodation_requests(request, owner_id):
    try:
        from HAC.models import AccommodationChangeRequest
        from HAC.serializers import AccommodationChangeRequestSerializer
        ...
        if status_filter == 'all':
            requests_list = AccommodationChangeRequest.objects.filter(owner__owner_id=owner_id)
        else:
            requests_list = AccommodationChangeRequest.objects.filter(owner__owner_id=owner_id, status=status_filter)
...
@api_view(['PUT'])
@jwt_required()
def update_accommodation_request_status(request, request_id):
    try:
        from HAC.models import AccommodationChangeRequest
        ...
        change_request = AccommodationChangeRequest.objects.filter(id=request_id).first()
        ...
        return Response({"status": True, "message": f"Request {new_status} successfully"}, status=status.HTTP_200_OK)
    except Exception as e:
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)