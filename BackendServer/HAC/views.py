...
def check_owner_status(request, phone):
    try:
        # Enforce owner isolation.
        #
        # One phone can own several accounts. When an owner registers a second
        # property, the waiting screen polls the status of THAT new account
        # while the app is still signed in to the first one. Overwriting the
        # requested id with the currently selected account reported on the
        # wrong account, so approval opened the old dashboard instead of the
        # newly approved one. Allow any account registered under the caller's
        # own phone, and answer about the account actually asked for.
        if request.jwt_payload.get('role') == 'owner':
            from HAC.services.common_service import CommonService
            owner_obj = getattr(request, 'owner_account', None) or request.custom_user
            if not owner_obj:
                return Response({"error": "You do not have permission to perform this action."}, status=status.HTTP_403_FORBIDDEN)
            target = CommonService.get_owner(phone)
            if not target or target.phone != owner_obj.phone:
                return Response({"error": "You do not have permission to perform this action."}, status=status.HTTP_403_FORBIDDEN)
            phone = target.owner_id
...
def update_request_status(request):
    try:
        ...
        if request.jwt_payload.get('role') == 'owner':
            ...
            owner_match = False
            if existing_req and (existing_req.owner == owner_obj or (existing_req.owner and owner_obj and existing_req.owner.phone == owner_obj.phone)):
                owner_match = True
            if join_req and (join_req.owner == owner_obj or (join_req.owner and owner_obj and join_req.owner.phone == owner_obj.phone)):
                owner_match = True
...
def add_owner_account(request):
    ...
    phone = request.jwt_payload.get('phone')
    if not phone or (len(str(phone)) == 10 and str(phone).isalnum() and not str(phone).isdigit()):
        user_id = request.jwt_payload.get('user_id')
        owner_obj = Owners.objects.filter(Q(owner_id=user_id) | Q(owner_id=phone) | Q(phone=user_id) | Q(phone=phone)).first()
        if owner_obj:
