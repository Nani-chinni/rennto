...
def get_jwt_signing_key():
    """
    Secret used to sign and verify this project's JWTs.

    Comes from settings.JWT_SIGNING_KEY, which is loaded from the
    environment. Falls back to settings.SECRET_KEY (also loaded from the
    environment) so an older settings module keeps working.
    """
...
def generate_jwt_token(user_id, role, phone=None):
    ...
    token = jwt.encode(payload, get_jwt_signing_key(), algorithm=get_jwt_algorithm())
    return token
...
def decode_jwt_token(token):
    ...
    """
    try:
        payload = jwt.decode(token, get_jwt_signing_key(), algorithms=[get_jwt_algorithm()])
        return payload
