...
class AccommodationChangeRequestSerializer(serializers.ModelSerializer):
    tenant_name = serializers.CharField(source='tenant.name', read_only=True)
    tenant_phone = serializers.CharField(source='tenant.phone', read_only=True)
    property_name = serializers.CharField(source='property.hostelName', read_only=True)
    
    class Meta:
        model = AccommodationChangeRequest
        fields = "__all__"
