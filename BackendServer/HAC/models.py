...
class AccommodationChangeRequest(models.Model):
    STATUS_CHOICES = [
        ('pending', 'Pending'),
        ('approved', 'Approved'),
        ('rejected', 'Rejected'),
    ...
    tenant = models.ForeignKey(Tenent, on_delete=models.CASCADE, related_name='accommodation_change_requests')
    property = models.ForeignKey(StayHostelDetails, on_delete=models.CASCADE, related_name='accommodation_change_requests')
    owner = models.ForeignKey(Owners, on_delete=models.CASCADE, related_name='accommodation_change_requests')
    
    change_type = models.CharField(max_length=50) # 'FLOOR', 'ROOM', 'BED'
    
    current_floor = models.CharField(max_length=50, blank=True, null=True)
    current_room = models.CharField(max_length=50, blank=True, null=True)
    current_bed = models.CharField(max_length=50, blank=True, null=True)
    
    requested_floor = models.CharField(max_length=50)
    requested_room = models.CharField(max_length=50)
    requested_bed = models.CharField(max_length=50)
    ...
    def __str__(self):
        tenant_name = self.tenant.name if self.tenant else "Tenant"
        return f"{tenant_name}: {self.change_type} Change ({self.status})"