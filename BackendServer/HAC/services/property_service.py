...
from HAC.validators import validate_upload_size
...
class PropertyService:
    ...
    def upload_gallery(request):
        ...
        paths = []
        for file in files:
            validate_upload_size(file, "Each property image")
    ...
    def create_property(request, owner, stay_type, facilities, gallery):
        ...
        if cover_image:
            validate_upload_size(cover_image, "Cover image")
    ...
    def manage_property_images(request, owner):
        ...
        if request.method == "GET":
            ...
            if action == "upload":
                files = request.FILES.getlist("images") or request.FILES.getlist("images[]") or list(request.FILES.values())
                paths = []
                import uuid
                for file in files:
                    fname = getattr(file, 'name', None) or f"camera_{uuid.uuid4().hex[:8]}.jpg"
                    if not fname.endswith(('.jpg', '.jpeg', '.png', '.heic', '.webp')):
                        fname += ".jpg"
                    path = default_storage.save(f"property_gallery/{fname}", file)
                    paths.append(path)
