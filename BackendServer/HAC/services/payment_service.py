    ...
    def create_payment(data):
        ...
        if owner:
            notif = Notification.objects.create(
                ...
                message=f"{tenant_name} paid ₹{data.get('amount')} for {data.get('property_name') or 'rent'}",
                type="PAYMENT",
            ...
            if owner.push_token:
                NotificationService.send_push_notification(
                    owner.push_token,
                    "Payment Received 💰",
                    f"{tenant_name} paid ₹{data.get('amount')}",
                )
            try:
                        ...
                        async_to_sync(channel_layer.group_send)(
                            group,
                            {
                                ...
                                "content": {
                                    ...
                                    "type": "PAYMENT",
                                    "title": "Payment Received 💰",
                                    "message": f"{tenant_name} paid ₹{data.get('amount')}",
                                    "is_read": False,
                                }
                            }
                        )
