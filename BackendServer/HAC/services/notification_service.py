...
class NotificationService:
    ...
    def send_tenant_notification(data):
        ...
        try:
            channel_layer = get_channel_layer()
            sanitized_phone = str(tenant_phone).replace("+", "").replace("@", "_").replace(".", "_").replace(" ", "")
            ws_content = {
                "id": notification.id,
    ...
    def get_unread_count(phone, role=None):
        ...
        if role == 'owner':
            ...
            if owner:
                if owner.phone:
                    phone_candidates.add(str(owner.phone).strip())
                    phone_candidates.add(str(owner.phone).strip().lstrip('+'))
                if owner.owner_id:
                    phone_candidates.add(str(owner.owner_id).strip())

                count = Notification.objects.filter(
                    Q(owner_account=owner) | Q(recipient_phone__in=list(phone_candidates)),
                    is_read=False
                ).count()
            else:
                count = Notification.objects.filter(recipient_phone__in=list(phone_candidates), is_read=False).count()
    ...
    def get_notifications(phone, role=None):
        ...
        if role == 'owner':
            ...
            if owner:
                ...
                notifications = Notification.objects.filter(recipient_phone__in=list(phone_candidates)).order_by('-created_at')
    ...
    def mark_all_notifications_read(phone, role=None):
        ...
        if role == 'owner':
            ...
            if owner:
                ...
                Notification.objects.filter(recipient_phone__in=list(phone_candidates), is_read=False).update(is_read=True)
        else:
            phone_variants = [clean_phone, clean_phone.lstrip('+')]
            if not clean_phone.startswith('+'):
                phone_variants.extend(['+' + clean_phone, '+91' + clean_phone, '91' + clean_phone])
