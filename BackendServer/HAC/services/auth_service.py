...
class AuthService:
    ...
    @staticmethod
    def send_otp(phone):
        phone = AuthService._normalize_phone(phone)
        ...
        demo_enabled = bool(system_settings and getattr(system_settings, 'demo_mode_enabled', False))
        raw_demo_numbers = system_settings.demo_mobile_numbers if (system_settings and system_settings.demo_mobile_numbers) else []
        demo_numbers = [AuthService._normalize_phone(num) for num in raw_demo_numbers if num]

        if demo_enabled and phone in demo_numbers:
            return {
        ...
        if not api_key:
            raise ValueError("SMS service configuration is missing.")
 
        url = (
            f"https://2factor.in/API/V1/"
            f"{api_key}/SMS/{phone}/AUTOGEN3/OTP1"
        )
        ...
        try:
            ...
            raise ValueError(f"SMS service error: {str(e)}")
    ...
    def verify_otp(phone, otp, role):
        ...
        if is_demo_number:
            ...
            verify_attempts = cache.get(f"otp_verify_attempts_{phone}", 0)
            if verify_attempts >= 5:
                cache.delete(f"otp_session_{phone}")
                cache.delete(f"otp_verify_attempts_{phone}")
                raise ValueError("Too many failed attempts. OTP has expired.")
            ...
            try:
                ...
                raise ValueError(f"Failed to verify OTP: {str(e)}")
            ...
            if data.get("Status") != "Success":
                ...
                if verify_attempts >= 5:
                    ...
                    cache.set(f"otp_verify_attempts_{phone}", verify_attempts, timeout=300)
                raise ValueError("Invalid OTP")
    ...
    ADMIN_PHONES = ["6281808454", "9347074726", "7672010079"]
