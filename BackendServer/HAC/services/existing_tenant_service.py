...
class RequestFields:
    """Reads a requested location field that different screens fill in
    differently."""
...
class ExistingTenantService:
    ...
    def create_request(data):
        ...
        # --- Describe the change in words the owner can act on ---
        from .request_service import RequestService

        current = RequestService.current_allocation(tenant, property_type)
        requested = {
            "floor": requested_floor,
            "room": requested_room or flat or section,
            "bed": requested_bed,
        }
        change_title, change_message, _current_text, _requested_text = RequestService.describe_change(
            tenant.name, property_type, current, requested
        )
        ...
        for p in [owner.owner_id, owner.phone]:
            if p:
                sanitized_phones.add(ExistingTenantService._sanitize_phone(p))

        for s_phone in sanitized_phones:
            ExistingTenantService._send_ws_notification(
                [f"owner_status_{s_phone}", f"user_notifications_{s_phone}", f"notifications_{s_phone}"],
                {
                    "id": notif.id,
    ...
    def _accept_hostel(req, building_layout, owner, tenant, today):
        ...
        requested_room = str(RequestFields.first(req.requested_room, req.requested_flat))
        requested_bed = str(req.requested_bed)
        if not requested_room:
            raise ValueError("This request does not name a room to move into.")
        ...
        occupant_bed = TenantBeds.objects.filter(
            Q(owner=owner) | Q(owner_phone=owner.owner_id),
            floor=ExistingTenantService._safe_int(requested_floor, 1),
            roomno=ExistingTenantService._safe_int(target_room.get("roomNo"), 1),
            bed=ExistingTenantService._safe_int(requested_bed, 1)
        ).first()
        ...
        parsed_db_room = ExistingTenantService._safe_int(db_room_no, 1)
        if len(str(db_room_no)) < 3:
            display_room_no = ExistingTenantService._safe_int(f"{requested_floor}{str(db_room_no).zfill(2)}", parsed_db_room)
        else:
            display_room_no = parsed_db_room
    ...
    def _accept_apartment(req, building_layout, owner, tenant, today):
        requested_floor = str(req.requested_floor)
        # The flat number arrives in different fields depending on which screen
        # sent the request: the property-details screen puts it in
        # requested_room, the My Stay accommodation-change picker puts it in
        # requested_flat. Reading only one of them made every change request
        # from the other screen fail with "Flat  not found on floor N." (400).
        requested_flat = str(
            RequestFields.first(req.requested_flat, req.requested_room)
        )
        if not requested_flat:
            raise ValueError("This request does not name a flat to move into.")
        ...
        occupant_flat = ApartmentTenantBeds.objects.filter(
            Q(owner=owner) | Q(owner_phone=owner.owner_id),
            floor=ExistingTenantService._safe_int(requested_floor, 1),
            flatno=ExistingTenantService._safe_int(target_flat.get("flatNo"), 1)
        ).first()
    ...
    def _accept_commercial(req, building_layout, owner, tenant, today):
        requested_floor = str(req.requested_floor)
        # Same split as apartments: requested_section from the accommodation
        # change picker, requested_room from the property-details screen.
        requested_section = str(
            RequestFields.first(req.requested_section, req.requested_room)
        )
        if not requested_section:
            raise ValueError("This request does not name a unit to move into.")
        ...
        occupant_sec = CommercialTenantBeds.objects.filter(
            Q(owner=owner) | Q(owner_phone=owner.owner_id),
            floor=ExistingTenantService._safe_int(requested_floor, 1),
            sectionNo=ExistingTenantService._safe_int(target_section.get("sectionNo"), 1)
        ).first()
