...
class VacateService:
    ...
    def approve_request(request_id):
        ...
        # Update previous join/stay requests to vacated so tenant details and status reset
        JoinRequest.objects.filter(
            tenant=tenant,
            status__in=['joined', 'completed', 'active', 'accepted', 'allotted', 'pending_confirmation']
        ).update(status='vacated')

        ExistingTenantRequest.objects.filter(
            tenant=tenant,
            status__in=['joined', 'completed', 'accepted', 'allotted']
        ).update(status='vacated')
